...
#include <ecnt_hook/ecnt_hook_ether_type.h>
...
static inline int  ETHER_MDIO_READ(unsigned int  phy_add, unsigned int  phy_reg){
    ...
    memset(&in_data, 0x0, sizeof(ECNT_ETHER_Data_s));
...
static inline int ETHER_API_GET_FLOW_CONTROL(unsigned char port_no, unsigned char *enable)
{
	...
	memset(&in_data, 0x0, sizeof(ECNT_ETHER_Data_s));
	in_data.function_id = ETHER_FUNCTION_GET_FLOW_CONTROL;
	in_data.ether_private.port_state.port_no = port_no;
...
static inline int ETHER_MDIO_CL45_READ(unsigned int phy_add, unsigned int dev_addr, unsigned int phy_reg)
{
    ECNT_ETHER_Data_s in_data;
    int ret=0;

    memset(&in_data, 0x0, sizeof(ECNT_ETHER_Data_s));
    in_data.function_id = ETHER_FUNCTION_MDIO_CL45_READ;
    in_data.ether_private.phy.add = phy_add;
    in_data.ether_private.phy.dev_add = dev_addr;
    in_data.ether_private.phy.reg = phy_reg;
    ret = __ECNT_HOOK(ECNT_ETHER_SWITCH, ECNT_DRIVER_API, (struct ecnt_data *)&in_data);
    if(ret != ECNT_HOOK_ERROR)
        return in_data.ether_private.phy.data;
...
static inline int ETHER_MDIO_CL45_WRITE(unsigned int phy_add, unsigned int dev_addr, unsigned int phy_reg, unsigned int phy_data)
{
    ECNT_ETHER_Data_s in_data;
    int ret=0;

    memset(&in_data, 0x0, sizeof(ECNT_ETHER_Data_s));
    in_data.function_id = ETHER_FUNCTION_MDIO_CL45_WRITE;
    in_data.ether_private.phy.add = phy_add;
    in_data.ether_private.phy.dev_add = dev_addr;
    in_data.ether_private.phy.reg = phy_reg;
    in_data.ether_private.phy.data = phy_data;   
