...
TARGET_DEVICES += bonanza

define Build/SrgFit

	$(STAGING_DIR_HOST)/bin/mkits-multiple-config.sh -o $@.its -A $(LINUX_KARCH) \
...
define Build/srgImageRun
	...
	$(CP) $(BIN_DIR)/$(IMG_PREFIX)-bonanza-fit-multi.itb $(KDIR)/img/Boot/fit-multi.itb
...
define Image/Flash/mkflash_emmc
    ...
    mkdir -p $$OUT_DIR; \
	mk_emmc_mfg_image.sh -e $(2) -r $(TARGET_DIR) -R $(BIN_DIR)/$(BINNAME).bin -b $(BIN_DIR) -c $$CDT_IPK -d $$OUT_DIR/$(IMG_PREFIX)-bonanza-emmc-mfg-$(2)-$(1).bin ; \
	cp $(STAGING_DIR_ROOT)/Boot/bin/preloader-emmc.$(2) $$OUT_DIR/$(IMG_PREFIX)-bonanza-preloader-emmc-mfg-$(2)-$(1).bin
endef
